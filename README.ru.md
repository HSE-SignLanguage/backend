...
Основные ошибки: `400` для некорректных полей формы, `408` для таймаута загрузки, `413` для размера, `415` только для неподдерживаемого или повреждённого media, `429`, когда video worker занят, и `503`, когда проверка прервана shutdown/cancellation либо исчерпана её ёмкость. Retryable-ответы `503` содержат `Retry-After`.
