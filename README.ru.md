...
Он принимает кадры из браузера по WebSocket или видео по HTTP, формирует фиксированные окна для ML, стабилизирует предсказания, опционально консервативно правит расшифровку через OpenRouter и возвращает append-only текст.
...
- **Realtime:** перекрывающиеся окна по 32 кадра с шагом 16, замена устаревшей работы свежей, подтверждение жеста на двух окнах и сообщения с расшифровкой по WebSocket.
...
Frontend по умолчанию использует same-origin `/api/`. Reverse proxy должен сохранять WebSocket upgrade для `/api/socket`. Swagger публикуется отдельно на `/swagger`, потому что UI загружает `/swagger/doc.json`; `SWAGGER_BASE_URL` задаёт только поле Swagger `host` и не должен содержать схему или путь.

Сгенерированный документ Swagger 2.0 описывает внутренние root-paths backend (`basePath: /`) и не кодирует внешний префикс gateway `/api`.
...
- жест публикуется после двух совпадающих принятых предсказаний;
- повтор того же жеста подавляется до двух отклонённых окон или `no`;
- ML-инференс и упорядоченный вывод OpenRouter/WebSocket разделены bounded queues.
...
Сервер отправляет text messages следующего вида:

```json
{
  "type": "transcript",
  "text": "новый append-only сегмент",
  "full_text": "актуальный снимок всей расшифровки",
  "confidence": 0.91
}
```

`text` — delta для обратной совместимости. Клиентам следует считать `full_text` авторитетным текущим состоянием вместо слепого добавления каждого сообщения.
...
Решение о распознавании всегда принимает ML. OpenRouter получает только принятый буквальный жест и до 1000 последних Unicode-символов контекста расшифровки — без кадров и загруженного видео.

Правка консервативна: нулевая температура, строгая JSON Schema, append-only проверка, ограниченные размеры запросов/ответов и таймаут пять секунд. Ответ может добавить только новую `delta` и не может переписать существующую расшифровку. При некорректном, обрезанном, слишком большом или недоступном ответе используется буквальный жест. Настроенная модель и provider должны поддерживать все запрошенные параметры structured output.
...
| `SWAGGER_BASE_URL` | `localhost:${BACKEND_PORT}` | Только Swagger host, например `hack.eferzo.xyz` |
...
GitHub Actions запускается для push в `main` и pull request с 15-минутным таймаутом job. Тесты покрывают валидацию расшифровки, ML-ответы и retry, стабилизацию предсказаний, trusted proxy, ёмкость video jobs и оконную обработку кадров.
...
- ограничения ответов ML/OpenRouter и строгая append-only валидация LLM;
- непривилегированный read-only container с удалёнными capabilities и ограниченным временным хранилищем;
- panic recovery, HTTP timeouts, структурированные логи, graceful HTTP shutdown и автоматическое удаление завершённых in-memory jobs через 24 часа.
...
- Установите deployment stop grace time больше десятисекундного бюджета HTTP shutdown с учётом ограничений ниже.
...
- `http.Server.Shutdown` не дренирует upgraded WebSocket и отделённые upload workers; деплой прерывает активные live-сессии и задачи.
...
- Правка OpenRouter не способна исправить неверно распознанный жест и может намеренно вернуть пустую delta, если безопасное добавление невозможно.
- Swagger описывает внутренние root-paths, а не внешний префикс `/api`.
